...
<template>
  <AppLayout>
    <div v-if="cart.length > 0" class="shoppingCart__container">
      <h2>Your Cart</h2>
      <ul class="shoppingCart__list">
        <li v-for="item in cart" :key="item.id" class="shoppingCart__list--item">
          ...
          <div class="shoppingCart__list--itemAction">
            <button @click="removeFromCart(item.id)" class="shoppingCart__list--itemAction__remove">Remove</button>
            <input
              type="number"
      ...
      <div class="shoppingCart__totalPrice">
        <strong>Total: {{ cart.reduce((sum, item) => sum + (item.tshirtDetails?.tshirt_price || item.tshirt.tshirt_price) * item.quantity, 0) }}€</strong>
      </div>
      <button @click="checkout" class="shoppingCart__checkout">CHECK OUT</button>
    </div>
